...
3. **스킬 문서 저작**: om-plan **PASS 확정**(2026-08-25 — 3차 CONDITIONAL→1차 검토(차단 2건 해소 확정)→사람 승인 예외 4차 감사 PASS→기계검증 3단 exit 0·FINAL_STATUS PASS). om-verify 1차 검토 승인(차단 0) 후 **사람 결정으로 한글 통일** → 한글 재빌드+새 감사 진행(지시서_om-verify_한글화_20260825.md). om-apply 한글화·wiring 한글 단언은 Codex 단계. om-verify 한글화 완료(6차 감사 PASS + Claude 재검증 승인 — 독립 사본 대조로 원본 진위 증명). Codex 2차 검토(85)가 **실환경 반례로 P0 2건**(feature ID 미필수→기존 ID 재사용 통과 / 실행 인터프리터 부재) 발견 → 사람 결정(feature ID 사람 필수 기재) → Codex 수정(87: 스키마 필수화+제안 대조, 표준 실행기 `harness/om`, 훅 정합, 276 테스트) → Claude 재검증 통과(88) → 스킬 2종에 반영+확인 감사 PASS·FINAL_STATUS 갱신(04:05Z). 이식 완료(90: digest 일치 이식 + om-apply 한글화 + wiring 한글 단언·삭제 반례) → Claude 최종 검증·커밋분리 3건(268→276→278 green)·push(91). **MR !2는 사람이 merge 완료(기존 6커밋 main 반영), 신규 3커밋은 MR !3 open.** 다음 = ① 사람 MR !3 merge ② 실기동 스모크(/om-plan 훅 발화 검증) ③ BANK-OM-005 스킬 경유 파일럿.
