...
> **★ 필수 규칙 — 사람 결정 요청 표준형식 (사용자 지시 2026-08-26·재지시 2026-09-02, 백로그 #9/개발요청 104).** 검사기가 block/analysis_error/approval 등 사람 결정이 필요한 결과를 낼 때, **절대로 항목만 나열하지 말고** 반드시 이 구조로 보고한다: ① **최상단 3줄 요약**(무슨 일/현재 상태/사람이 할 일) ② **쉬운 설명**("즉, ~입니다" 의역) ③ **선택지** (A)/(B)/(C) ④ **각 선택의 결과** ⑤ **권고 + 주체 명시 + 근거**("권고: X. 권고 주체: 저. 이유: ..."). 사람은 구조를 모를 수 있으므로 세션이 대신 골라 제시해야 한다. (아직 SKILL.md/검사기에 코드로 강제 안 됨 → 세션이 반드시 관행으로 지킬 것. 정식 강제는 개발요청 104.)
...
3. **스킬 문서 저작**: om-plan **PASS 확정**(2026-08-25 — 3차 CONDITIONAL→1차 검토(차단 2건 해소 확정)→사람 승인 예외 4차 감사 PASS→기계검증 3단 exit 0·FINAL_STATUS PASS). om-verify 1차 검토 승인(차단 0) 후 **사람 결정으로 한글 통일** → 한글 재빌드+새 감사 진행(지시서_om-verify_한글화_20260825.md). om-apply 한글화·wiring 한글 단언은 Codex 단계. om-verify 한글화 완료(6차 감사 PASS + Claude 재검증 승인 — 독립 사본 대조로 원본 진위 증명). Codex 2차 검토(85)가 **실환경 반례로 P0 2건**(feature ID 미필수→기존 ID 재사용 통과 / 실행 인터프리터 부재) 발견 → 사람 결정(feature ID 사람 필수 기재) → Codex 수정(87: 스키마 필수화+제안 대조, 표준 실행기 `harness/om`, 훅 정합, 276 테스트) → Claude 재검증 통과(88) → 스킬 2종에 반영+확인 감사 PASS·FINAL_STATUS 갱신(04:05Z). 이식 완료(90: digest 일치 이식 + om-apply 한글화 + wiring 한글 단언·삭제 반례) → Claude 최종 검증·커밋분리 3건(268→276→278 green)·push(91). **MR !2는 사람이 merge 완료(기존 6커밋 main 반영), 신규 3커밋은 MR !3 open.** **MR !3도 merge 완료(사람, 2026-08-25)** — 도구·스킬·장부 전부 GitLab main 반영. **제품 저장소 확인: `datacatalog/kb-datacatalog` main=`8ac18ad053`(커스텀 1.13.1+BANK-OM-001~007, 로컬 리허설 start_ref와 동일)** — 사내 3종 세트(제품·도구·장부) 완비. **업그레이드 실전 진행 중(사내 서버, 가이드 94A v2)**: 훅 순서 P0 실기동 발견→당일 핫픽스(95·96, MR !4 merge) → **/om-plan 합격(exit 2·사유 0, 2026-08-26)** — attempt 3회(append-only), 판독 16→10→0건, 사람 결정 2회(24), digest 보관 장치 3회 검증. 시연자료/ 유형별 수집(S1~S6·B1~B3·I1~I3·D1~D4) + 개선 백로그 97(9건). 다음 = 사람 계획 검토(Q-001 등) → /om-apply → /om-verify → 결과 95. 병행 열린 항목: test-agent(UI 부품)의 GitLab 반입 여부 — 당장 불필요(UI 부품은 선택 슬롯·meta.run_id 미결로 미결속 상태), UI 부품 실사용 전에 사람 결정.
3-구. (경위) **스킬 문서 저작(진행 중)**: 스킬저장소에 claude-skill-author 메타스킬 설치 완료(2026-08-24). 사용자가 스킬저장소에서 새 세션을 열어 **없는 스킬 2종을 저작**: `지시서_om-plan_스킬저작_20260824.md` → `지시서_om-verify_스킬저작_20260824.md` 순서(om-apply는 SKILL.md 기보유). 이후 Codex 검토 → 검사기 저장소 이식(MR 합류). 내용 요구 정본은 83 부록 A(+76·81).
4. **om-report(4단계) 설계 착수 여부 — 사람 결정.** (설계→적대검토→구현 순서 유지.)
5. 기록된 후속: P0-2 활성잠금(R-1·R-2와 함께)·P1-2~5·upgrade 독립 문서검토 배선(리허설 66·68)·om-verify 남은위험 6건(81).
