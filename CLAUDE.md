...
- **/om-verify(검증, 3단계): 설계+적대검토+구현지시서 완료(2026-08-24), 구현 착수 대기.** 설계 `73`·적대검토 `75`(P0 5건: 테스트 재계산·전체 인계·빌드 영수증·skip 차단·receipt 신뢰) → **구현 정본 `76`**. 본체(pytest, 옛 실행기 pytest_runs/testruns 이관) + UI층 부품 슬롯. **test-agent 채택 확정**(사람 결정 — OM lab 파일럿 통과 doc 72, 정형 I/O 계약 뒤 교체가능 구조). 다음 = 설계 적대검토 + V-1~V-3 사람 결정(환경 기동 A/B/C 등).
- **/om-report: 미착수.**
