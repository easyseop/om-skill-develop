...
3. **스킬 문서 저작**: om-plan **PASS 확정**(2026-08-25 — 3차 CONDITIONAL→1차 검토(차단 2건 해소 확정)→사람 승인 예외 4차 감사 PASS→기계검증 3단 exit 0·FINAL_STATUS PASS). om-verify 1차 검토 승인(차단 0) 후 **사람 결정으로 한글 통일** → 한글 재빌드+새 감사 진행(지시서_om-verify_한글화_20260825.md). om-apply 한글화·wiring 한글 단언은 Codex 단계. 다음 = 한글화 완료 후 **Codex 2차 검토 지시서** 작성.
